...
- **Color is redundant, never the sole signal** (CVD / `NO_COLOR` / piped output degrade to the text status). The `●` dot reinforces the status *text*, which stays UNCOLORED — the dot carries the status color; coloring the word too would double-encode the same axis (over-coloring).
- **TWO orthogonal colored axes** — the `!` column is NOT a function of status (it carries undischarged-stop count + per-factor issue severity, independent of the lifecycle verdict). (1) the **dot** = status color; (2) the **`!` markers** = severity color. Keep both; no information lost.
...
- **Truecolor HEX, not ANSI names** (ANSI `blue`→purple, `yellow`→amber through the SVG theme palette — the meaning must not bend to the palette). Status→color map keyed on `StatusKind` + terminal `Outcome`:
  - `live` → `#3fb950` · `stale`/`preempted`/`conflicted` → `#d29922` · `pending`/`missing` → `#8b949e` · `corrupt`/`unreadable`/`fold-error`(=`StatusKind.ERROR`)/`errored`/`killed`/`presumed_dead` → `#f85149` · `completed` → `#539bf5`.
- **`_marker` is severity-colored + cleanly split** — it returns a colored `Text`: `⚠⚠` `#f85149` (HIGH), `⚠` `#d29922` (MEDIUM), `■N` neutral (default). It keys on the **issue** severity + stop count ONLY (`max((i.severity for i in row.issues), default=OK)` + `row.undischarged_stops`), NOT `row.severity` — so `corrupt`/`unreadable` (HIGH *status* severity) stop double-signalling a `⚠⚠` (the dot already carries them). `corrupt` still shows `⚠⚠` (its `_corrupt` plants a HIGH *issue*); `unreadable` (a bare row, no issue) shows no marker — its red dot + text carry it.
